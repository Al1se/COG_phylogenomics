#!/usr/bin/env python3
"""Download mmCIF files from RCSB for tokens like 5TJG_D or 6UU8_DDD."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download RCSB mmCIF files for token lists")
    parser.add_argument("token_list", help="Text file with one PDB_chain token per line")
    parser.add_argument(
        "-o",
        "--output-dir",
        required=True,
        help="Directory for downloaded .cif files",
    )
    ...
    for token in tokens:
        ...
        out_path = output_dir / f"{entry_id}.cif"
        if out_path.exists():
        ...
        for attempt in range(1, args.retries + 1):
            try:
                ...
                print(f"Downloaded: {entry_id}.cif")
                break
